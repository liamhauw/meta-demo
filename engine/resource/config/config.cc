...
Config::Config() {
  std::ifstream config_file{config_file_path_.generic_string()};
  std::string line;
  while (std::getline(config_file, line)) {
    ...
    if (seperate_pos > 0 && seperate_pos < (line.length() - 1)) {
      ...
      if (name == "world") {
        world_file_path_ = asset_path_ / "world" / value;
      }
    }
  }
}

const std::filesystem::path& Config::GetWorldFile() const { return world_file_path_; }
