...
class Rendering {
 ...
 private:
  ...
  static std::pair<vk::Result, uint32_t> AcquireNextImageWrapper(
      const vk::raii::SwapchainKHR& swapchain, uint64_t timeout, vk::Semaphore semaphore,
      vk::Fence fence) {
  ...
  void CopyToDevice(const vk::raii::DeviceMemory& device_memory, const T* data,
    ...
    uint8_t* device_data{static_cast<uint8_t*>(device_memory.mapMemory(0, buffer_size))};
    memcpy(device_data, data, buffer_size);
    device_memory.unmapMemory();
  }

  void CopyBuffer(const vk::raii::Buffer& src_buffer, const vk::raii::Buffer& dst_buffer,
                  vk::DeviceSize size);
